...
import { LeafBackground } from "@/components/effects/leaf-background";
import { useGranary } from "@/lib/store";
...
function LoginPage() {
  ...
  const handleLogin = async (e?: React.FormEvent) => {
    ...
    if (authEnabled && emailAndPasswordEnabled) {
      ...
      try {
        ...
        const { checkAuthRateLimit } = await import("@/lib/server/phone-otp");
        ...
        if (error) {
          const { recordAuthAttempt, logAuditEvent } = await import("@/lib/server/phone-otp");
        ...
        // Authenticated
        const { resetAuthRateLimit, logAuditEvent } = await import("@/lib/server/phone-otp");
  ...
  async function persistProfileAndDocs(
    ...
    try {
      const { upsertProfile, uploadDocument } = await import("@/lib/server/granary");
      await upsertProfile({
        data: {
