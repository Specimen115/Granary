...
export const useGranary = create<GranaryState>((set, get) => ({
  ...
  farmersList: seedFarmers,
  operatorsList: seedOperators,
  lots: seedLots,
  selectedId: null,
  mapFilter: "all",
  query: "",
  ...
  hydrateFromDb: (payload) =>
    set((state) => ({
      ...
      farmersList: payload.farmersList?.length
        ? payload.farmersList
        : state.farmersList,
      operatorsList: payload.operatorsList?.length
  ...
  refreshFromDb: async () => {
    try {
      const { loadCatalog } = await import("@/lib/server/granary");
